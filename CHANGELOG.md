...
- 展开老播客不再误报"中央没抓字幕"：`展开第 N 个播客` 走新脚本 `scripts/fetch_transcript.py`，按 `guid`（或标题）直接从中央 feed 现取字幕，不再依赖本地缓存文件。根因是 `prepare_digest.py` 每次运行都会清空 `transcripts/` 目录，叠加"已读"过滤把当天已推送的集从 payload 里剔除——早上推送时写下的字幕文件，到下午你想展开时已被删且不会重写，于是 Agent 找不到文件、回你"中央没抓"，但中央 feed 里字幕其实一直在。改成现取后，无论是否已读都能展开；若某集确实没有字幕（该频道未配字幕抓取、RSS 也没带），脚本会明确告知"本来就没抓、无内容可展开"（退出码 2），不再和"抓了但本地被删"混为一谈。
